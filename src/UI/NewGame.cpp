...
GameStateType NewGame::update(float deltaTime)
{
    ...
    m_oneShotEffects.erase(std::remove_if(m_oneShotEffects.begin(), m_oneShotEffects.end(), [](const auto& e)
    {
        return e->isFinished();
...
{
    auto pvpBounce = std::make_unique<UI::ScaleEffect>(m_pvpBtn.getSprite(), 0.f, BOUNCE_DURATION, BOUNCE_SCALE);
    ...
    auto botBounce = std::make_unique<UI::ScaleEffect>(m_pvbotBtn.getSprite(), 0.f, BOUNCE_DURATION, BOUNCE_SCALE);
    ...
    auto backBounce = std::make_unique<UI::ScaleEffect>(m_backBtn.getSprite(), 0.f, BOUNCE_DURATION, BOUNCE_SCALE);
...
{
    auto pvpGrow = std::make_unique<UI::ScaleEffect>(m_pvpBtn.getSprite(), 0.3f, 0.f, 1.f);
    pvpGrow->pop_init();
    m_oneShotEffects.push_back(std::move(pvpGrow));

    auto botGrow = std::make_unique<UI::ScaleEffect>(m_pvbotBtn.getSprite(), 0.3f, 0.f, 1.f);
    botGrow->pop_init();
    m_oneShotEffects.push_back(std::move(botGrow));

    auto backGrow = std::make_unique<UI::ScaleEffect>(m_backBtn.getSprite(), 0.3f, 0.f, 1.f);
    backGrow->pop_init();
