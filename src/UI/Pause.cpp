...
{
    for (auto* btn : m_allButtons) btn -> update(m_window);
    ...
    m_oneShotEffects.erase(std::remove_if(m_oneShotEffects.begin(), m_oneShotEffects.end(),
        [](const auto& effect) { return effect->isFinished(); }), m_oneShotEffects.end());
...
void PauseMenu::draw()
{
    ...
    for (auto* btn : m_allButtons) btn -> draw(m_renderTexture);
...
sf::Sprite& PauseMenu::getRenderSprite() { return m_renderSprite; }

void PauseMenu::createUI()
{
    auto resumeBounce = std::make_unique<UI::ScaleEffect>(m_resumeBtn.getSprite(), 0.f, 0.15f, 1.15f);
    m_resumeBtn.setOnClick([this, ptr = resumeBounce.get()]() { ptr -> trigger(); this -> onResumeClick(); });
    m_persistentEffects.push_back(std::move(resumeBounce));


    auto resetBounce = std::make_unique<UI::ScaleEffect>(m_resetBtn.getSprite(), 0.f, 0.15f, 1.15f);
    m_resetBtn.setOnClick([this, ptr = resetBounce.get()]() { ptr -> trigger(); this -> onResetClick(); });
    m_persistentEffects.push_back(std::move(resetBounce));


    auto saveBounce = std::make_unique<UI::ScaleEffect>(m_saveGameBtn.getSprite(), 0.f, 0.15f, 1.15f);
    m_saveGameBtn.setOnClick([this, ptr = saveBounce.get()]() { ptr -> trigger(); this -> onSaveGameClick(); });
    m_persistentEffects.push_back(std::move(saveBounce));


    auto settingsBounce = std::make_unique<UI::ScaleEffect>(m_settingsBtn.getSprite(), 0.f, 0.15f, 1.15f);
    m_settingsBtn.setOnClick([this, ptr = settingsBounce.get()]() { ptr -> trigger(); this -> onSettingsClick(); });
    m_persistentEffects.push_back(std::move(settingsBounce));


    auto quitBounce = std::make_unique<UI::ScaleEffect>(m_quitToMenuBtn.getSprite(), 0.f, 0.15f, 1.15f);
    m_quitToMenuBtn.setOnClick([this, ptr = quitBounce.get()]() { ptr -> trigger(); this -> onQuitToMenuClick(); });
...
void PauseMenu::onResumeClick() { m_requestedState = GameStateType::GoBack; }
void PauseMenu::onResetClick() { m_requestedState = GameStateType::ResetGame; }
void PauseMenu::onSaveGameClick()
{
    m_requestedState = GameStateType::SaveGameRequest;
}
void PauseMenu::onSettingsClick() { m_requestedState = GameStateType::Settings; }
void PauseMenu::onQuitToMenuClick() { m_requestedState = GameStateType::MainMenu; }
