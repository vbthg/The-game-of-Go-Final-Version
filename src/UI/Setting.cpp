...
void Setting::createUI()
{
    ...
    m_stoneTheme->setOnChange([this](int i)
    {
        onStoneThemeChange(i);
    });

    m_boardTheme = std::make_unique<UI::RadioGroup>(3, sf::Vector2f{486.f, 451.f}, 78.5f);
    m_boardTheme->setOnChange([this](int i)
    {
        onBoardThemeChange(i);
    });

    m_musicTheme = std::make_unique<UI::RadioGroup>(3, sf::Vector2f{486.f, 654.f}, 78.5f);
    m_musicTheme->setOnChange([this](int i)
    {
        onMusicThemeChange(i);
    });
    ...
    m_musicSlider->setOnValueChange([this](float v)
    {
    ...
    m_sfxSlider->setOnValueChange([this](float v)
    {
        onSfxVolumeChange(v);
    });
    ...
    m_timeLimit->setOnChange([this](int idx)
    {
        onTimeLimitChange(idx);
    });
    ...
    m_komiPoints->setOnChange([this](int idx)
    {
        onKomiChange(idx);
    });
...
void Setting::onStoneThemeChange(int index)
{
    ...
    saveSetting();
    GlobalSetting::getInstance().notifyThemeChanged();
}
...
void Setting::onMusicThemeChange(int index)
{
    ...
    gs.musicThemeIndex = index;

    float currentVol = gs.musicVolume;
    ResourceManager::getInstance().setMusicVolume(currentVol);

    ResourceManager::getInstance().playMusic(index);

//    std::cout << "[Setting]: music theme index " << index << "\n";
