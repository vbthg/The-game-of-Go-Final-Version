...
void SizeSelection::createUI()
{
    ...
    m_9x9Btn.setOnClick([this](){ on9x9Click(); });
    m_13x13Btn.setOnClick([this](){ on13x13Click(); });
    m_19x19Btn.setOnClick([this](){ on19x19Click(); });
    ...
    if(m_currentMode == GameMode::PlayerVsAI)
    {
        ...
        m_easyBtn.setOnClick([this](){ onEasyClick(); });
        m_medBtn.setOnClick([this](){ onMedClick(); });
        m_hardBtn.setOnClick([this](){ onHardClick(); });

        m_groupDiffBtns = { &m_easyBtn, &m_medBtn, &m_hardBtn };
    }


    auto playBounce = std::make_unique<UI::ScaleEffect>(m_playBtn.getSprite(), 0.f, 0.15f, 1.15f);
    m_playBtn.setOnClick([this, e = playBounce.get()](){ e -> trigger(); onPlayClick(); });
    m_effects.push_back(std::move(playBounce));

    m_backBtn.setOnClick([this](){ onBackClick(); });
...
GameStateType SizeSelection::update(float deltaTime)
{
    if(!m_introEffects.empty())
    {
        ...
        m_introEffects.erase(std::remove_if(m_introEffects.begin(), m_introEffects.end(),
            [](const auto& effect) { return effect -> isFinished(); }), m_introEffects.end());
...
// --- Callbacks ---
void SizeSelection::on9x9Click()   { m_selectedSize = 9; }
void SizeSelection::on13x13Click() { m_selectedSize = 13; }
void SizeSelection::on19x19Click() { m_selectedSize = 19; }

void SizeSelection::onEasyClick()  { m_selectedDiff = AiDifficulty::Easy; }
void SizeSelection::onMedClick()   { m_selectedDiff = AiDifficulty::Medium; }
void SizeSelection::onHardClick()  { m_selectedDiff = AiDifficulty::Hard; }

void SizeSelection::onPlayClick() { m_requestedState = GameStateType::GamePlay; }
void SizeSelection::onBackClick() { m_requestedState = GameStateType::NewGame; }

int SizeSelection::getSelectedSize() const { return m_selectedSize; }
AiDifficulty SizeSelection::getSelectedDifficulty() const { return m_selectedDiff; }
